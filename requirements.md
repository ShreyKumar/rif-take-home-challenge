...
8. **Frontend language — TypeScript, not vanilla JS** *(revises the original Phase 6 decision)*: the
   ...
   the frontend now requires `cd frontend && npm install && npm run build` first (see plan.md Phase 11).
...
│   ├── index.html
│   ├── app.js                # generated by `npm run build` (tsc) — gitignored, not committed
│   ├── styles.css
