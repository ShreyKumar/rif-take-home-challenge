...
| Frontend | Static **HTML + vanilla JS + minimal CSS** in `frontend/`, served by the backend at `/` via `http.FileServer` | No build step, no framework; served from disk (see §9) |
...
- **FR-5.6** Styling is **minimal, hand-written CSS** — no framework, no build step.
...
- A UI framework or frontend build pipeline.
...
```
.
├── frontend/                 # pure static assets — no build step, no Go
│   ├── index.html
│   ├── app.js
│   └── styles.css
